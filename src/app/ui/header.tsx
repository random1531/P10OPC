...
export default function header() {
    ...
    return (
        <header className="flex items-center justify-between h-24 w-full bg-white pt-2 pb-2 pr-24 pl-24">
            <Image src={Logo} width={147} height={19} alt="logo" className="flex align-middle" />

            <div className="flex items-center gap-4">
                <div className={"flex items-center gap-4 w-60 h-20 justify-center rounded-xl" + (Dash ? " bg-black text-white" : "bg-white text-orange-500")}>
                    <MdDashboard />
                    <span><Link href="/dashboard">Tableau de bord</Link></span>
                </div>
                <div className={"flex items-center gap-4 w-60 h-20 justify-center rounded-xl" + (Project ? " bg-black text-white" : "bg-white text-orange-500")}>
                    <FaFolderOpen />
                    <span><Link href="/project">Projets</Link></span>
                </div>
            </div>
