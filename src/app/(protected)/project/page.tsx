...
export default function Project() {
    ...
    useEffect(() => {
        async function loadTasks() {
            ...
            const groupedTasks = results.reduce<Record<string, Task[]>>((acc, item) => {
    ...
    return (
        <div className="flex flex-col w-full items-center gap-16 mt-6">
            ...
            <div className="grid grid-cols-3 gap-12 flex-wrap w-4/5">
                {projects.map((e) => (
                    <button className="cursor-pointer flex flex-col bg-white rounded-xl pt-7 pb-7 pr-8 pl-8 border gap-14 justify-between border-gray-200" key={e.id} onClick={() => router.push(`/project/${e.id}`)}>
                        <ProjectCard Description={e.description} Name={e.name} />
                        <div className="flex flex-col gap-3.5 w-full">
                            <ProjectProgress
                                done={(taskProject[e.id] || []).filter((t) => t.status === "DONE").length}
                                total={(taskProject[e.id] || []).length}
                            />
                            <div className="flex items-center gap-1 font-normal text-[10px] text-gray-600"><AiOutlineTeam /> Equipe ({e.members.length + 1})</div>
                            <div className="flex gap-2 items-center">
                                <p className="text-black text-xs h-7 w-7 rounded-full bg-[#FFE8D9] flex items-center justify-center">{e.owner.name.substring(0, 2).toUpperCase()}</p>
                                <div className="bg-[#FFE8D9] text-[#D3590B] rounded-2xl p-1 flex items-center justify-center">Propriétaire</div>

                                <div className="flex">
                                    {e.members.filter((m) => m.user.name !== userOwn).map((m) => (
                                        <p className="text-black text-xs h-7 w-7 rounded-full bg-gray-200 flex items-center justify-center" key={m.id}>
