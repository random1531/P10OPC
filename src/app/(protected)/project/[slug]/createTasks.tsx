...
export default function createTasks() {
    ...
    const Badge = [
        { status: "LOW", color: 'red', textremplacement: 'Basse', bgColor: "#FFE0E0" },
        { status: "MEDIUM", color: 'orange', textremplacement: 'Normal', bgColor: "#FFF0D7" },
        { status: "HIGH", color: 'green', textremplacement: 'Haute', bgColor: "#F1FFF7" },
    ]
    useEffect(() => {
        fetchProjects();
    }, [])
    ...
    return (
        <div className="flex flex-col gap-8">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4" action="">
                <InputFunction valueInput={title} idvalue="title" type="text" onchange={(e) => setTitle(e.target.value)} labelText="Titre" />
                <InputFunction valueInput={description} idvalue="description" type="text" onchange={(e) => setDescription(e.target.value)} labelText="Description" />
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex gap-2">
                        {Badge.map((item) => (<span onClick={() => setPriority(() => (item.status))} className="cardStyle" style={{ color: item.color, backgroundColor: item.bgColor, padding: "4px 8px", fontSize: "12px", fontWeight: "bold", height: "fit-content", borderRadius: '50px', display: "flex", justifyContent: "center", alignItems: "center" }} key={item.status}>{item.textremplacement}</span>))}
                    </div>
                </div>
