            ...
            fetchAssignedTasks: async () => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        const tasksAssigned = Array.isArray(data?.takssAssigned) ? data.takssAssigned : [];
                        set({ tasksAssigned, loading: false });
                    } else {
                        set({ 
                            tasksAssigned: [], 
                            loading: false, 
                            error: result.message || "Erreur lors du chargement des tâches assignées" 
                        });
                    }
                } catch (err: any) {
                    set({ 
                        loading: false, 
                        error: err.message || "Erreur lors du chargement des tâches assignées" 
                    });
                }
            }
        }),
