...
type ProjectTasksState = {
  ...
  addTasks: (id: string, title: string, description: string, priority: string, dueDate: string, assigneeIds: string[]) => Promise<void>
...
export const useProjectTasksStore = create<ProjectTasksState>()(
  devtools(
    (set, get) => ({
      ...
      addTasks: async (id: string, title: string, description: string, priority: string, dueDate: string, assigneeIds: string[]) => {
        set({ loading: true, error: null });
        try {
          const result = await AddTasksToproject({ id, title, description, priority, dueDate, assigneeIds });
          if (result?.success) {
